...
public class ITCrossEngineDremio {
  ...
  public static final String DREMIO_TABLE = "from_dremio";
  public static final String SPARK_TABLE = "from_spark";
  public static final String FLINK_TABLE = "from_flink";
  ...
  private static Dataset<Row> sparkSql(@Language("SQL") String query, Object... args) {
    ...
    try {
      ...
      throw new RuntimeException("Spark failed to exec query: " + fullQuery, e);
    }
  }
  ...
  private static String sparkTable(String tableName) {
    return "nessie.db." + tableName;
  }

  private static String dremioTable(String tableName) {
    return Stream.of(dremioHelper.getCatalogName(), "db", tableName)
        .map(x -> '"' + x + '"')
        .collect(Collectors.joining("."));
  }

  private static String flinkTable(String tableName) {
    return Stream.of(dremioHelper.getCatalogName(), tableName)
        .map(x -> '`' + x + '`')
        .collect(Collectors.joining("."));
  }
  ...
  private static void selectFromSpark(List<List<Object>> tableRows, String testTable) {
    assertThat(
        sparkSql("SELECT id, val FROM %s", sparkTable(testTable))
            .collectAsList()
            .stream()
  ...
  private static void selectFromDremio(List<List<Object>> tableRows, String testTable) {
    List<List<Object>> rows = dremioHelper.runSelectQuery("SELECT * FROM %s", dremioTable(testTable));
